...
v_caract_ejemplo=np.zeros((5,12))

for i in range(len(corpus_vehiculos_electricos)):

    v_caract_ejemplo[i][0]=(caract.contar_palabras(corpus_vehiculos_electricos[i]))
    embeddig3d=caract.generar_embedding_3d(corpus_vehiculos_electricos[i])
    v_caract_ejemplo[i][1]=embeddig3d[0]
    v_caract_ejemplo[i][2]=embeddig3d[1]
    v_caract_ejemplo[i][3]=embeddig3d[2]
    v_caract_ejemplo[i][4]=(caract.tf_palabra_mas_comun(corpus_vehiculos_electricos[i]))
    v_caract_ejemplo[i][5]=(caract.idf_ultima_palabra(corpus_vehiculos_electricos[i],corpus_vehiculos_electricos))
    v_caract_ejemplo[i][6]=(caract.tfidf_palabra_medio(corpus_vehiculos_electricos[i],corpus_vehiculos_electricos))
    v_caract_ejemplo[i][7]=(caract.contar_arrobas(corpus_vehiculos_electricos[i]))
    v_caract_ejemplo[i][8]=(caract.contar_hash(corpus_vehiculos_electricos[i]))
    v_caract_ejemplo[i][9]=(caract.contar_verbos(corpus_vehiculos_electricos[i]))
    v_caract_ejemplo[i][10]=(caract.contar_adjetivos(corpus_vehiculos_electricos[i]))
    v_caract_ejemplo[i][11]=(caract.calcular_polaridad(corpus_vehiculos_electricos[i]))



"""Exportar el archivo CSV"""
# Ruta del archivo CSV de salida
ruta_archivo_csv = 'matriz_caracteristicas_ejemplo.csv'
